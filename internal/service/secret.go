...
// SecretService — бизнес-логика работы с секретами (шифрование, валидация, репозиторий).
...
// Create создаёт секрет: валидирует payload по типу, шифрует и сохраняет. Возвращает id или ErrValidation.
...
// GetByID возвращает секрет с расшифрованными данными. При отсутствии — ErrSecretNotFound.
...
// Update обновляет данные секрета: валидирует payload по текущему типу, шифрует и сохраняет.
...
// Delete помечает секрет как удалённый. При отсутствии — ErrSecretNotFound.
